(function(global) {
    ...
    function onTestEnd(test, error) {
        ...
        if (error) {
            ...
            var output,
                ...
                trace = error.stack || message,
                index = trace.indexOf(message);

            if (index === -1) {
                output = message;
            } else {
                index += message.length;
                output = trace.slice(0, index);
                trace = trace.slice(index + 1);
            }

            if (error.uncaught) {
                output = 'Uncaught ' + output;
            }

            log(INDENT + output);
            log(trace.split('\n').map(function(str, i) {
                return (i === 0 ? indent() : indent() + indent()) + str.trim();
            }).join('\n'));
            console.log('');
        ...
        var nodenode = createTestNode(test);

        last(stack).nodes.push(nodenode);
        stat(test.state);
    ...
    function getResult() {
        ...
        try {
            json = JSON.stringify(stack[0], null, 2);
        } catch(e) {
            console.log(e.stack);
        }
    ...
    function clean(obj) {
        ...
        ['title', 'message', 'state', 'error', 'root'].forEach(function(k) {
            if (obj[k]) {
                o[k] = obj[k];
            }
        });
