...
function TreeJSON(runner) {
    ...
    runner.on('suite', function(suite) {
        ...
        var node = {
            type: 'suite',
            data: _.assign(cleanSuite(suite), {
                tests: 0,
                failed: 0,
                passed: 0,
                duration: 0
...
function tree(node, parent) {
    if (node.type !== 'test') {
        ...
        parent.data.tests += node.tests;
        parent.data.passed += node.passed;
        parent.data.failed += node.failed;
        parent.data.duration += node.duration;
    } else {
        parent.data.tests +=1;
